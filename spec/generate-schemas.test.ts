...
const expectedSpec = {
  'info': {'title': 'Cats example', 'description': 'The cats API description', 'version': '1.0'},
  ...
  'components': {
    'schemas': {
      ...
      'CatSearch': {'properties': {'name': {'type': 'string'}, 'breed': {'type': 'string'}, 'age': {'type': 'number'}}},
      'Cat': {'properties': {'name': {'type': 'string'}, 'breed': {'type': 'string'}}},
      'Todo': {
        'properties': {
          'title': {'type': 'string'},
    ...
    '/second-endpoint/{id}/{name}': {
      'get': {
        ...
        'parameters': [ {
          ...
          'required': true,
          'schema': {'type': 'string'}
        }, {'name': 'name', 'in': 'path', 'description': '', 'required': true, 'schema': {'type': 'string'}} ]
      }
    },
    '/hello': {
      'get': {
    ...
    '/my-endpoint': {
      'get': {
        ...
        'parameters': [ {
          ...
          'required': false,
          'schema': {'type': 'string'}
        }, {'name': 'age', 'in': 'query', 'description': '', 'required': false, 'schema': {'type': 'number'}} ]
      },
      'post': {
...
Deno.test('host swagger', async (ctx) => {
  ...
  await ctx.step('serve swagger json file on given endpoint/json', async () => {
    const listenEvent = await app.listen(0);
    const response = await fetch(`http://localhost:${listenEvent.port}${swaggerPath}/json`);
